...
public class GenericService<TEntity> : IGenericService<TEntity> where TEntity : class, new()
{
    ...
    public void Remove(int id)
    {
        ...
        if (idProperty is null) throw new Exception("The entity dosn't have Id field!");
        idProperty.SetValue(tEntity, id, null);
        _uow.MarkAsDeleted(tEntity);
    }

    public TEntity FindById(int id) => _entity.Find(id);

    public async Task<TEntity> FindByIdAsync(int id) => await _entity.FindAsync(id);

    public List<TEntity> GetAll() => _entity.ToList();
